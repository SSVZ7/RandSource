    ...
    > Node<E, SPOK, SSIG>
{
    ...
    pub fn share_pvss<R: Rng>(
        ...
        let mut f = (0..=self.aggregator.config.degree)
            .map(|_| E::Fr::rand(rng))
            .collect::<Vec<_>>();
        //println!("participants.len()={}", self.aggregator.participants.len());
        ...
        //debug y_eval_i
        //println!("y_eval_i.len()={}", y_eval_i.len());
        
        //Y_1, ..., Y_n
        // getting ech party's public_key_sig and raising to a_i
        let y_i = y_eval_i[0..self.aggregator.participants.len()]
            .iter()
            .enumerate()
